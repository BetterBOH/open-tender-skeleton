...
export default ({ fill, alt, uniqueAriaId }) => (
  <svg
    ...
    height="26"
    viewBox="0 0 26 26"
    aria-labelledby={`bag-${uniqueAriaId}`}
  >
    <desc id={`bag-${uniqueAriaId}`}>{alt || 'Bag'}</desc>
    <defs>
      <path
        id={`bag-a-${uniqueAriaId}`}
    ...
    <g fill="none" fillRule="evenodd">
      <mask id={`bag-b-${uniqueAriaId}`} fill="#fff">
        <use xlinkHref={`#bag-a-${uniqueAriaId}`} />
      </mask>
      <g fill={fill} mask={`url(#bag-b-${uniqueAriaId})`}>
